...
// JobNftCreate 处理链上Nft到钱包
func (s *WalletJob) JobNftCreate() {
	log.GetLogger().Debug("job JobNftCharge Success ", zap.Any("Topic", s.KafkaConfig.TopicNftCreate))
	go func() {
		for err := range s.NftCreateConsumer.Errors() {
	...
	go func() {
		for {
			...
			if err := s.NftCreateConsumer.Consume(s.ConsumerCtx, []string{s.KafkaConfig.TopicNftCreate}, hand); err != nil {
...
type KafkaNftCreateJob struct {
	walletSrv walletPb.WalletSrvServer
}

func (consumer KafkaNftCreateJob) Setup(_ sarama.ConsumerGroupSession) error {
	return nil
}

func (consumer KafkaNftCreateJob) Cleanup(_ sarama.ConsumerGroupSession) error {
	return nil
}

func (consumer KafkaNftCreateJob) ConsumeClaim(sess sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
	var err error
	var ctx = context.TODO()
	for message := range claim.Messages() {
		var result scanCommon.Nft721CreateForPush
