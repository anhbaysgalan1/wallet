...
func (c *walletCache) BalanceSave(ctx context.Context, balance []*entity.Balance) error {
	...
	for _, item := range balance {
		...
		value[item.Currency] = string(item.Balance)
	}
...
func (c *walletCache) BalanceSysSet(ctx context.Context, sysUid uint64, amount, currency string, isAdd bool) (entity.Amount, entity.Amount, error) {
	...
	if err != nil {
		if !errors.Is(err, redis.Nil) {
			log.GetLogger().Error("[BalanceSysSet] c.BalanceGetByUid failed", zap.Uint64("sys uid", sysUid), zap.Error(err))
			return "", "", err
		}
	}
	...
	// 这里没有判断系统用户账户余额是否充足
	if isAdd {
		afterBalance, err = afterBalance.Add(amount)
		if err != nil {
			return "", "", err
		}
	} else {
		var result bool
		afterBalance, result, err = afterBalance.Sub(amount)
		if err != nil {
			return "", "", err
		}
		if !result {
	...
	// 修改系统余额
	err = c.cache.HSet(common.KeyWalletSysBalance(sysUid), currency, string(afterBalance)).Err()
	if err != nil {
