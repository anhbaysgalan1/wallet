...
func (r RecordChargeGetReq) ToString() string {
	...
	if r.Timestamp <= 0 {
		r.Timestamp = tool2.GetTimeUnixMilli()
	}
	...
	if r.Status > 0 {
		values.Add("status", tool2.IntToStr(int(r.Status)))
	}
	if r.StartTime > 0 {
		values.Add("startTime", tool2.Int64ToStr(r.StartTime))
	}
	if r.EndTime > 0 {
		values.Add("endTime", tool2.Int64ToStr(r.EndTime))
	}
	if r.Offset > 0 {
		values.Add("offset", tool2.IntToStr(r.Offset))
	}
	if r.Limit > 0 {
		values.Add("limit", tool2.IntToStr(r.Limit))
	}
	values.Add("recvWindow", tool2.Int64ToString(r.RecvWindow))
	values.Add("timestamp", tool2.Int64ToStr(r.Timestamp))
